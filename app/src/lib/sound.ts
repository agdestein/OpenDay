...
const GAP: Partial<Record<SoundName, number>> = { thud: .09, splash: .15, click: .04, tick: .3, ding: .06, flood: .4, alarm: 1.5, horn: 1.2, clack: .035, whoosh: .08, sizzle: .25, boing: .12, squeak: .08, pop: .03 };
...
const RECIPES: Record<SoundName, (a: AudioContext, t: number, pitch: number, volume: number) => void> = {
  ...
  boing: (a, t, p, v) => { tone(a, t, 170 * p, .16, .22 * v, 'triangle', 560 * p); tone(a, t + .12, 560 * p, .28, .12 * v, 'sine', 330 * p); },
  squeak: (a, t, p) => { tone(a, t, 950 * p, .09, .12, 'sine', 1700 * p); tone(a, t + .07, 1500 * p, .07, .08, 'sine', 1150 * p); },
  pop: (a, t, p, v) => { tone(a, t, 520 * p, .07, .16 * v, 'sine', 1250 * p); hiss(a, t, .04, .06 * v, 3000, 1500, 2); },
};
