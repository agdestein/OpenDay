...
in vec2 vUv;
out vec4 outColor;
uniform sampler2D uA, uB, uFog, uLand, uLut;
...
export class MapGL {
  ...
  private tex: Record<'A' | 'B' | 'Fog' | 'Land' | 'Lut', WebGLTexture>;
  ...
  private constructor(land: Uint8Array, nx: number, ny: number, private widthKm: number, private heightKm: number) {
    ...
    gl.useProgram(prog);
    ['uA', 'uB', 'uFog', 'uLand', 'uLut'].forEach((name, unit) => gl.uniform1i(this.u(name), unit));
  }
