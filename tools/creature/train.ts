...
import { Evolution, randomShoves, rewardScore, seededRandom, withReflexes, type Reward } from '../../app/src/games/creature/evolve';
...
const round = (g: Genome): Genome => ({
  ...
  ...(g.net ? { net: { hidden: g.net.hidden, w: g.net.w.map((x) => +x.toFixed(4)) } } : {}),
});
...
// Brains that feel: a Doggo practising with shoves, as a rhythm brain and with senses (reflexes).
// Both start from the park's Doggo; each is the run closest to the median of 8 on the push test.
const pushTests = Array.from({ length: 20 }, (_, k) => randomShoves(12, seededRandom(9000 + k)));
const pushed = (genome: Genome) => {
  const runs = pushTests.map((q) => simulate(doggo, genome, 12, {}, q));
  return { flips: runs.filter((c) => c.turns() > 0.4).length, dist: runs.reduce((a, c) => a + c.dist(), 0) / runs.length };
};
const shoveTrain = (feel: boolean) =>
  [0, 1, 2, 3, 4, 5, 6, 7].map((r) => {
    const seed = 41000 + r;
    const start = feel ? withReflexes(trained.Doggo!, doggo, seededRandom(seed)) : trained.Doggo!;
    const evo = new Evolution(doggo, { start, rand: seededRandom(seed), evalTime: 8, shovesFor: (gen) => randomShoves(8, seededRandom(seed * 1000 + gen)) });
    // 100 generations: with 50, only about half the runs with senses learn to use them.
    evo.runGenerations(100);
    // The steadiest of the last generation's best five on four fresh shove sequences.
    const checks = [1, 2, 3, 4].map((k) => randomShoves(8, seededRandom(seed * 31 + k)));
    let best = evo.best!.genome;
    let bestScore = -Infinity;
    for (const cand of evo.lastTop) {
      const sc = checks.reduce((a, q) => a + rewardScore(simulate(doggo, cand, 8, {}, q), 'far'), 0);
      if (sc > bestScore) {
        bestScore = sc;
        best = cand;
      }
    }
    const genome = round(best);
    return { genome, ...pushed(genome) };
  });
const byMedianFlips = (runs: { genome: Genome; flips: number; dist: number }[]) => {
  const m = median(runs.map((r) => r.flips));
  return runs.reduce((a, b) => (Math.abs(b.flips - m) < Math.abs(a.flips - m) || (Math.abs(b.flips - m) === Math.abs(a.flips - m) && b.dist > a.dist) ? b : a));
};
const shovedRuns = shoveTrain(false);
const feelRuns = shoveTrain(true);
const shoved = byMedianFlips(shovedRuns);
const feel = byMedianFlips(feelRuns);
console.log(`push test (flips of 20): park Doggo ${pushed(trained.Doggo!).flips}; shove-practised rhythm ${shovedRuns.map((r) => r.flips).join(' ')} → ${shoved.flips}; with senses ${feelRuns.map((r) => r.flips).join(' ')} → ${feel.flips}`);

const g = (x: Genome) =>
  `{ freq: ${x.freq}, muscles: [${x.muscles.map((m) => `{ amp: ${m.amp}, phase: ${m.phase} }`).join(', ')}]${x.net ? `, net: { hidden: ${x.net.hidden}, w: [${x.net.w.join(', ')}] }` : ''} }`;
out.push(
